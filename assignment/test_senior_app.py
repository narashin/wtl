...
def test_company_name_autocomplete(api):
    ...
    assert searched_companies == [{'name': '주식회사 링크드코리아', 'id': 3}, {'name': '스피링크', 'id': 4}]
...
def test_company_search(api):
    ...
    """
    resp = api.get(
        "/corp/Wantedlab", headers={"x-wanted-language": "ko"}
    )
    ...
    resp = api.get(
        "/corp/없는회사", headers={"x-wanted-language": "ko"}
    )
...
def test_search_tag_name(api):
    ...
    """
    resp = api.get("/tags?query=タグ_4", headers={"x-wanted-language": "ko"})
    searched_companies = resp.json()

    assert [company["name"] for company in searched_companies] == [
        "원티드랩",
        "OKAY.com"
    ]
